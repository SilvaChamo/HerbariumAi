...
const MarketDashboard: React.FC<MarketDashboardProps> = ({ company, onClose, onEdit }) => {
    return (
        <div className="p-6 pb-20 space-y-6 animate-in fade-in slide-in-from-bottom">
            ...
            <div className="bg-white p-3 rounded-lg border border-slate-100 shadow-sm flex items-center gap-3 active:scale-95 transition-all">
                <div className={`w-10 h-10 ${company.plan === 'Produtor' ? 'bg-emerald-50 text-emerald-600' : 'bg-orange-50 text-orange-600'} rounded-lg flex items-center justify-center text-base`}>
                ...
                <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                        {company.products.map((p, i) => (
                            <div
                                key={i}
                                className="flex justify-between items-center text-xs p-2 bg-emerald-50/30 rounded-lg"
                            >
            ...
            <button
                onClick={onEdit}
                className="w-full py-4 bg-slate-900 text-white rounded-lg font-bold text-sm shadow-xl active:scale-95 transition-all"
            >
